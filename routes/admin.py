...
from Models.DAO import DAO_INSTANCE as DAO
...
def book_add():
	...
	if request.method == 'GET':
		...
		qty = str(_form["qty"])
		available = 1 if str(_form["available"]) else 0
		desc = str(_form["desc"])

		book_manager.add(title, qty, available, desc)
...
def book_edit(id):
	...
	if request.method == 'GET':
		...
		book_manager.update(id, title, qty, available, desc)
