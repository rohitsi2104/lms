class UserDAO():
	...
	def list(self):
		users = self.db.query("select "
							  "MIN(@table.id) AS id,"
							  "MIN(@table.name) AS name,"
							  "MIN(@table.email) AS email,"
							  "MIN(@table.bio) AS bio,"
							  "MIN(@table.mob) AS mob,"
							  "MIN(@table.lock) AS `lock`,"
							  "MIN(@table.created_at) AS created_at,"
							  "count(reserve.book_id) as books_owned "
							  "from @table LEFT JOIN reserve ON reserve.user_id=@table.id GROUP BY reserve.user_id").fetchall()
