class BookDAO():
	...
	def add(self, title, qty, available, desc):
		q = self.db.query(f"""
			INSERT INTO `books` (`name`, `desc`, `author`, `availability`, `edition`, `count`) VALUES
			('{title}', '{desc}', 'Default Author', {available}, '1', {qty})
		""")
		self.db.commit()

		return q

	def update(self, id, title, qty, available, desc):
		q = self.db.query(f"""
			UPDATE `books`
			SET `name` = '{title}', `desc` = '{desc}', `availability` = {available}, `count` = {qty}
			WHERE `id` = {id}
		""")
		self.db.commit()
